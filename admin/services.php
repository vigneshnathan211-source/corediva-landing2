<?php
/**
 * Services: per-country page basics (service_content) and the section
 * stack (service_page_sections) a service-detail page is built from.
 * One service + one country (+ region) is in scope at a time, picked via
 * the selector form below -- same GET-driven pattern as settings.php's
 * tabs, just with two extra dimensions to pick before a tab means anything.
 */
...
require_permission($admin, 'services.view');
$canEdit = in_array('services.edit', $admin['permissions'], true);

// Admin needs to see (and re-activate) inactive services too, so this
// queries the table directly rather than reusing the public get_services()
...
if (!$services || !$countries) {
    ...
    exit('Services or countries are not configured.');
}

$serviceId = (int) ($_GET['service'] ?? $services[0]['id']);
...
if (!$service || !$country) {
    ...
    exit('Unknown service or country.');
}

$regions = db_all('SELECT * FROM regions WHERE country_id = ? AND is_active = 1 ORDER BY name', [$countryId]);
if ($regionId > 0 && !array_filter($regions, static fn ($r) => (int) $r['id'] === $regionId)) {
    $regionId = 0;
}

$tab = in_array($_GET['tab'] ?? '', ['info', 'basics', 'sections'], true) ? $_GET['tab'] : 'basics';
$qs  = "service={$serviceId}&country={$countryId}&region={$regionId}";

$errors = [];
$notice = null;

/** Build the ordered items[] JSON for a section from repeated form rows, shape depends on $type. */
function services_admin_build_items(string $type, array $post): ?array
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!$canEdit) {
        ...
        // -----------------------------------------------------------
        // Service info (services -- country-independent base row)
        // -----------------------------------------------------------
        if ($action === 'save_info') {
            $title      = trim((string) ($_POST['title'] ?? ''));
            $slug       = trim((string) ($_POST['slug'] ?? ''));
            $icon       = trim((string) ($_POST['icon'] ?? '')) ?: 'layout';
            $category   = trim((string) ($_POST['category'] ?? '')) ?: null;
            $shortDesc  = trim((string) ($_POST['short_description'] ?? '')) ?: null;
            $coreDesc   = trim((string) ($_POST['core_description'] ?? '')) ?: null;
            $coreDeliv  = trim((string) ($_POST['core_deliverables'] ?? '')) ?: null;
            ...
            if (!$errors && db_value('SELECT COUNT(*) FROM services WHERE slug = ? AND id <> ?', [$slug, $serviceId]) > 0) {
            ...
            if (!$errors) {
                ...
                // Re-fetch so the rest of the request (form, dropdown, page
                // title) reflects the save instead of the pre-update row --
                // matters especially for is_active, since the dropdown query
                // above ran before this UPDATE.
                $services = db_all('SELECT * FROM services ORDER BY sort_order');
                foreach ($services as $s) {
                    if ((int) $s['id'] === $serviceId) {
                        $service = $s;
        ...
        // -----------------------------------------------------------
            ...
            $compliance  = trim((string) ($_POST['compliance_note'] ?? '')) ?: null;
            $ctaText     = trim((string) ($_POST['cta_text'] ?? '')) ?: null;
            ...
            db_exec(
                'INSERT INTO service_content
                 ...
                 ON DUPLICATE KEY UPDATE
                    ...
                    local_proof = VALUES(local_proof), compliance_note = VALUES(compliance_note),
                    cta_text = VALUES(cta_text), meta_title = VALUES(meta_title),
                    meta_description = VALUES(meta_description), is_published = VALUES(is_published)',
                [$serviceId, $countryId, $regionId, $title, $h1, $intro, $localProof, $compliance, $ctaText, $metaTitle, $metaDesc, $isPublished]
            );
            admin_audit((int) $admin['id'], 'update', 'service_content', $serviceId, $service['slug'] . '/' . $country['code']);
            $notice = 'Page basics saved.';

        // -----------------------------------------------------------
        // Sections (service_page_sections)
        // -----------------------------------------------------------
            ...
            if (!array_key_exists($sectionType, SERVICE_SECTION_TYPES)) {
                $errors[] = 'Choose a valid section type.';
            }

            $items = services_admin_build_items($sectionType, $_POST);
            $itemsJson = $items !== null ? json_encode($items, JSON_UNESCAPED_UNICODE) : null;

            if (!$errors && $action === 'create_section') {
                // New sections always join at the end of the stack -- ordering
                // afterwards is drag-and-drop only (see 'reorder_sections' below).
                $sortOrder = (int) db_value(
                    'SELECT COALESCE(MAX(sort_order), 0) FROM service_page_sections WHERE service_id = ? AND country_id = ? AND region_id = ?',
                    [$serviceId, $countryId, $regionId]
                ) + 10;

                db_exec(
                    'INSERT INTO service_page_sections
                        (service_id, country_id, region_id, section_type, heading, subheading, body, items, media_url, media_alt, image_position, cta_label, cta_url, sort_order, is_published)
                     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                    [$serviceId, $countryId, $regionId, $sectionType, $heading, $subheading, $body, $itemsJson, $mediaUrl, $mediaAlt, $imagePosition, $ctaLabel, $ctaUrl, $sortOrder, $isPublished]
                );
                $newId = (int) db()->lastInsertId();
                admin_audit((int) $admin['id'], 'create', 'service_page_sections', $newId, $sectionType);
                $notice = 'Section added.';
            } elseif (!$errors) {
                $target = db_one('SELECT * FROM service_page_sections WHERE id = ? AND service_id = ? AND country_id = ? AND region_id = ?', [$id, $serviceId, $countryId, $regionId]);
                if ($target === null) {
                    $errors[] = 'That section no longer exists.';
                } else {
                    // sort_order is untouched here -- position only changes via drag-and-drop.
                    db_exec(
                        'UPDATE service_page_sections SET
                    ...
                    admin_audit((int) $admin['id'], 'update', 'service_page_sections', $id, $sectionType);
            ...
            if ($target === null) {
                ...
                db_exec('DELETE FROM service_page_sections WHERE id = ?', [$id]);
                admin_audit((int) $admin['id'], 'delete', 'service_page_sections', $id, (string) $target['section_type']);
        ...
        // -----------------------------------------------------------
            ...
            $validIds = array_map('intval', array_column(
                db_all('SELECT id FROM service_page_sections WHERE service_id = ? AND country_id = ? AND region_id = ?', [$serviceId, $countryId, $regionId]),
                'id'
            ));
            ...
            if (!$order) {
                ...
                try {
                    ...
                    foreach ($order as $sectionId) {
                        db_exec('UPDATE service_page_sections SET sort_order = ? WHERE id = ?', [$sort, $sectionId]);
                        $sort += 10;
                    }
                    db()->commit();
                    admin_audit((int) $admin['id'], 'update', 'service_page_sections', null, 'reordered ' . count($order) . ' section(s)');
...
$basics = db_one('SELECT * FROM service_content WHERE service_id = ? AND country_id = ? AND region_id = ?', [$serviceId, $countryId, $regionId]);

$editingSection = null;
if ($tab === 'sections' && isset($_GET['edit'])) {
    $editingSection = db_one('SELECT * FROM service_page_sections WHERE id = ? AND service_id = ? AND country_id = ? AND region_id = ?', [(int) $_GET['edit'], $serviceId, $countryId, $regionId]);
}

$sectionsList = db_all('SELECT * FROM service_page_sections WHERE service_id = ? AND country_id = ? AND region_id = ? ORDER BY sort_order', [$serviceId, $countryId, $regionId]);
...
<?php require __DIR__ . '/includes/admin-nav.php'; ?>
    ...
    <h1>Services</h1>
    <p class="cd-admin-lede">Pick a service and a country to edit that page's H1/intro/meta, then build its body as an ordered stack of sections.</p>

<?php if (!$canEdit): ?>
    <div class="cd-admin-alert cd-admin-alert-info" role="status"><p>You have view-only access here. Changes are saved by someone with the services.edit permission.</p></div>
<?php endif; ?>
<?php if ($notice): ?>
    ...
    <section class="cd-admin-panel">
        <form method="get" class="cd-admin-form-row cd-admin-selector">
            <div>
                ...
                <select id="sel-service" name="service" onchange="this.form.submit()">
<?php foreach ($services as $s): ?>
                    <option value="<?= (int) $s['id'] ?>" <?= (int) $s['id'] === $serviceId ? 'selected' : '' ?>><?= esc($s['title']) ?></option>
<?php endforeach; ?>
                </select>
            </div>
            <div>
...
<?php if ($regions): ?>
            <div>
                ...
                <select id="sel-region" name="region" onchange="this.form.submit()">
                    <option value="0" <?= $regionId === 0 ? 'selected' : '' ?>>&mdash; Country-wide &mdash;</option>
<?php foreach ($regions as $r): ?>
                    <option value="<?= (int) $r['id'] ?>" <?= (int) $r['id'] === $regionId ? 'selected' : '' ?>><?= esc($r['name']) ?></option>
<?php endforeach; ?>
                </select>
            </div>
<?php endif; ?>
    ...
    <div class="cd-admin-tabs">
        <a href="<?= esc(admin_url('services.php?' . $qs . '&tab=info')) ?>" class="cd-admin-tab<?= $tab === 'info' ? ' is-active' : '' ?>">Service info</a>
        <a href="<?= esc(admin_url('services.php?' . $qs . '&tab=basics')) ?>" class="cd-admin-tab<?= $tab === 'basics' ? ' is-active' : '' ?>">Page basics</a>
        <a href="<?= esc(admin_url('services.php?' . $qs . '&tab=sections')) ?>" class="cd-admin-tab<?= $tab === 'sections' ? ' is-active' : '' ?>">Sections (<?= count($sectionsList) ?>)</a>
    </div>

<?php if ($tab === 'info'): ?>

    <section class="cd-admin-panel">
        <h2>Service info &mdash; <?= esc($service['title']) ?></h2>
        <p class="cd-admin-lede">The country-independent base row: title, slug, icon, category and the
           client-supplied long-form copy. This is what every country's page falls back to.</p>
        ...
        <form method="post" class="cd-admin-form">
            ...
            <div class="cd-admin-form-row">
                <div>
                    ...
                    <input type="text" id="info_title" name="title" required value="<?= esc($service['title']) ?>">
                </div>
                <div>
                    <label for="info_slug">Slug</label>
                    <input type="text" id="info_slug" name="slug" required value="<?= esc($service['slug']) ?>">
                </div>
            </div>
            ...
            <div class="cd-admin-form-row">
                <div>
                    ...
                    <input type="text" id="info_icon" name="icon" value="<?= esc($service['icon']) ?>" placeholder="las la-code">
                </div>
                <div>
                    <label for="info_category">Category (mega-menu column)</label>
                    <input type="text" id="info_category" name="category" value="<?= esc($service['category'] ?? '') ?>">
                </div>
                <div>
                    <label for="info_sort">Sort order</label>
                    <input type="number" id="info_sort" name="sort_order" value="<?= (int) $service['sort_order'] ?>">
                </div>
            </div>

            <div>
                <label for="info_short">Short description (one line, used on cards/grids)</label>
                <input type="text" id="info_short" name="short_description" maxlength="500" value="<?= esc($service['short_description'] ?? '') ?>">
            </div>
            ...
            <div class="cd-admin-form-actions">
                <button type="submit" class="cd-admin-btn">Save service info</button>
            </div>
        </form>
<?php endif; ?>
    </section>
...
<?php elseif ($tab === 'basics'): ?>
    ...
    <section class="cd-admin-panel">
        <h2>Page basics &mdash; <?= esc($service['title']) ?> / <?= esc($country['name']) ?></h2>
        <p class="cd-admin-lede">The H1, intro and meta tags for this page. Leave a field blank to fall back to the service's default copy.</p>
<?php if ($canEdit): ?>
        <form method="post" class="cd-admin-form">
            <input type="hidden" name="csrf_token" value="<?= esc(csrf_token()) ?>">
            ...
            <div class="cd-admin-form-row">
                <div>
                    ...
                    <input type="text" id="title" name="title" value="<?= esc($basics['title'] ?? '') ?>" placeholder="<?= esc($service['title']) ?>">
                </div>
                <div>
                    <label for="h1">H1</label>
                    <input type="text" id="h1" name="h1" value="<?= esc($basics['h1'] ?? '') ?>" placeholder="<?= esc($service['title']) ?>">
                </div>
            </div>
            ...
            <div class="cd-admin-form-row">
                <div>
                    ...
                    <input type="text" id="cta_text" name="cta_text" value="<?= esc($basics['cta_text'] ?? '') ?>">
                </div>
            </div>
...
<?php foreach (SERVICE_SECTION_TYPES as $key => $label): ?>
...
<?php if ($editingSection): ?>
                <a href="<?= esc(admin_url('services.php?' . $qs . '&tab=sections')) ?>" class="cd-admin-btn-ghost">Cancel</a>
<?php endif; ?>
            </div>
        </form>
    </section>
...
<?php endif; ?>
                        ...
                        <td><?= esc(SERVICE_SECTION_TYPES[$sec['section_type']] ?? $sec['section_type']) ?></td>
...
<?php endif; ?>
                        ...
                        <td>
<?php if ($canEdit): ?>
                            <a href="<?= esc(admin_url('services.php?' . $qs . '&tab=sections&edit=' . $sec['id'])) ?>">Edit</a>
                            &nbsp;
...
<?php endforeach; ?>
<?php if (!$sectionsList): ?>
                    <tr><td colspan="5">No sections yet for this service/country.</td></tr>
<?php endif; ?>
                </tbody>
            </table>
        </div>
