...
CREATE TABLE `countries` (
  ...
  `dial_code`       VARCHAR(8)   NOT NULL DEFAULT '+1' COMMENT 'phone prefix shown on lead forms',
...
CREATE TABLE `nav_items` (
  ...
  `mega_type`    VARCHAR(20)  DEFAULT NULL COMMENT 'services|products: build the panel from that table instead of child rows',
  `sort_order`   INT          NOT NULL DEFAULT 0,
  `is_active`    TINYINT(1)   NOT NULL DEFAULT 1,
  PRIMARY KEY (`id`),
